...
class FinishReadingView(APIView):
    permission_classes = [permissions.IsAuthenticated]

    def post(self, request, pk):
        log = get_object_or_404(
            ReadingLog.objects.select_related("book").prefetch_related("book__authors"),
            pk=pk,
            user=request.user,
        )

        serializer = FinishReadingSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        with transaction.atomic():
            log.status = "finished"
            log.finished_at = serializer.validated_data["finished_at"]
            log.save()
        ...
        return Response(
            {
                "log": ReadingLogSerializer(log, context={"request": request}).data,
...
class FriendsListView(APIView):
    ...
    def get(self, request):
        following_relationships = list(
            Follow.objects.filter(follower=request.user)
            .select_related("following")
            .order_by("following__username")
        )
        follower_relationships = list(
            Follow.objects.filter(following=request.user)
            .select_related("follower")
            .order_by("follower__username")
        )

        follower_id_set = {relationship.follower_id for relationship in follower_relationships}
        following_id_set = {relationship.following_id for relationship in following_relationships}
        ...
        for relationship in following_relationships:
            ...
            if relationship.following_id in follower_id_set:
                payload["follower_user_id"] = relationship.following_id
                friends.append(payload)
            else:
                following_only.append(payload)
...
class MyProfileView(APIView):
    ...
    def get(self, request):
        ...
        current_reads = (
            ReadingLog.objects.filter(user=request.user, status="reading")
            .select_related("book")
            .prefetch_related("book__authors")
            .order_by("-updated_at")
        )
        ...
        recent_reads = (
            ...
            .select_related("book")
            .prefetch_related("book__authors")
            .order_by("-finished_at", "-updated_at")[:5]
        )
        ...
        return Response(
            {
                ...
                "currently_reading": ReadingLogSerializer(current_reads, many=True).data,
...
class RecommendationView(APIView):
    ...
    def get(self, request):
        ...
        recommendations = (
            recommendations.filter(recommendation_filter)
            .distinct()
            .prefetch_related("authors")[:12]
        )
